...
\begin{itemize}
    ...
    \item Each method is run under four seeds (\texttt{-{}-seeds 0 1 2 3}), set with \texttt{set\_seed} over PyTorch, NumPy, and Python \texttt{random}; per-seed metrics are appended to \texttt{results\_3d\_seeds.csv} and aggregated into the report's table, figure, and significance test by \texttt{aggregate\_seeds.py}
    \item Per-epoch training curves are written to \texttt{logs\_3d/}, one file per method and seed
\end{itemize}

One caveat should be stated plainly. The run seed controls weight initialization and batch shuffling, and the train/validation split is held fixed across seeds, so our four seeds give a genuine estimate of run-to-run variance for evaluation IoU. One source of randomness remains unseeded: the surface sampling behind Chamfer-L1 and normal consistency draws from the global random state at evaluation time. Those two metrics therefore carry a small additional variance we do not quantify, which is why we decline to interpret differences between methods at the fourth decimal place.
